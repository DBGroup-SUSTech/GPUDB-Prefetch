...
TEST(measure, DISABLED_naive) {
  ...
  // assert(r_n <= s_n);
  std::string r_fname = cutil::rel_fname(true, "r_uniq", r_n, 0, r_n);
  std::string s_fname = cutil::rel_fname(true, "s_uniq", s_n, 0, std::max(s_n, r_n));
...
TEST(unique, naive) {
  ...
  int s_max_id = s_n > r_n ? s_n : r_n;

  std::string r_fname = cutil::rel_fname(true, "r", r_n, 0, r_n);
  std::string s_fname = cutil::rel_fname(true, "s", s_n, 0, s_max_id);
  int32_t *r_key = new int32_t[r_n];
  int32_t *s_key = new int32_t[s_n];
  ...
  assert(!datagen::create_relation_unique(r_fname.c_str(), r_key, r_n, r_n));
  assert(
      !datagen::create_relation_unique(s_fname.c_str(), s_key, s_n, s_max_id));
...
TEST(skew_r_unique_s, naive) {
  ...
  int32_t s_maxid = r_n > s_n ? r_n : s_n;

  std::string r_fname = cutil::rel_fname(false, "r", r_n, skew, r_n);
  std::string s_fname = cutil::rel_fname(true, "s", s_n, 0, s_maxid);
  int32_t *r_key = new int32_t[r_n];
  int32_t *s_key = new int32_t[s_n];
  ...
  assert(
      !datagen::create_relation_unique(s_fname.c_str(), s_key, s_n, s_maxid));
...
TEST(skew_s_unique_r, naive) {
  ...
  std::string r_fname = cutil::rel_fname(true, "r", r_n, 0, r_n);
  std::string s_fname = cutil::rel_fname(false, "s", s_n, skew, s_maxid);
  int32_t *r_key = new int32_t[r_n];
  int32_t *s_key = new int32_t[s_n];

  // generate key
  assert(!datagen::create_relation_zipf(s_fname.c_str(), s_key, s_n, s_maxid,
                                        skew));
  assert(!datagen::create_relation_unique(r_fname.c_str(), r_key, r_n, r_n));

  // fmt::print(cutil::fmt_arr(s_key, s_n, true));

...
gp = [[] for _ in range(c)]
spp = [[] for _ in range(len(spp_range))]
amac = [[] for _ in range(c)]
imv = [[] for _ in range(c)]

name = 'F' # B1: 4 B2: only spp
...
data_spp = [np.mean(spp[i]) for i in range(len(spp_range))]
...
def line_plot(source, save_path, title = "",x_process = lambda x:x):
    ...
    for legend in source["legends"]:
        ...
        r = []
        if source["legend_names"][legend] == 'baseline':
            continue
        elif source["legend_names"][legend] == 'SPP':
            r = np.arange(len(spp_range))
        else:
            r = xrange
...
line_plot(source, 'Figure15.pdf', "Lanes per warp")
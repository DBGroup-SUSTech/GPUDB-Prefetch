...
struct ConfigIMV : public Config {
  int method = 3; // all lane has its own states in shared memory
};
...
__global__ void probe_ht_2stage_regs(Tuple *s, int s_n, EntryHeader *ht_slot,
  ...
  while (all_done < PDIST) {
    ...
    case state_t::NEXT: {
      ...
      if (active_cnt + rvs_cnt < 32) { // empty
        ...
        unsigned long tmp_s_tuple =
            __shfl_sync(MASK_ALL_LANES, rvs_reg.s_tuple, offset);
        Entry *tmp_next = (Entry *)__shfl_sync(
  ...
  // handle RVS
  if (warplane < rvs_cnt) {
    unsigned long s_tuple = rvs_reg.s_tuple;

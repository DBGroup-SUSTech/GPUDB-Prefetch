...
__global__ void probe_ht(int32_t *s_key, int32_t *s_payload, int32_t s_n, int32_t *r_key, int32_t *r_payload,
                         int32_t *ht_link, int32_t *ht_slot, int32_t ht_size_log, int32_t *o_aggr)
{
    ...
    for (size_t i = tid; i < s_n; i += stride * G)
    {
        ...
        int32_t finish_match_num = 0;
#pragma unroll
        // code 0, get s's key and payload, do hash to calculate the slot_num, prefetch the hashcell
        for (size_t j = 0; j < G; j++)
        {
            // set the state to HASH
            reg_state[j] = state_t::HASH;
            ...
            // prefetch ht_slot[slot_num]
            pref.commit(&shared_buffer[shared_buffer_idx], &ht_slot[slot_num], sizeof(int32_t));
            // change the sate to NEXT
            reg_state[j] = state_t::NEXT;
        }

        while (finish_match_num != G)
        {
#pragma unroll
            // code 1, if the bucket is not empty, prefetch key of R
            for (size_t j = 0; j < G; j++)
            {
                s_tuple_id = i + j * stride;
                switch (reg_state[j])
...
#pragma unroll
            // code 2, match the keys.
            // if mathched, prefetch payload of R, set state to PAYLOAD.
            // if not, link to next hashcell, set state to NEXT
            for (size_t j = 0; j < G; j++)
            {
                s_tuple_id = i + j * stride;
                switch (reg_state[j])
...
#pragma unroll
            // code 3, get payload of R and do aggregation or output the payload to output buffer.
            for (size_t j = 0; j < G; j++)
            {
                s_tuple_id = i + j * stride;
                switch (reg_state[j])
                ...
                case state_t::PAYLOAD:
                    ...
                    pref.commit(&shared_buffer[shared_buffer_idx], &ht_link[reg_r_tuple_id[j]], sizeof(int32_t));
                    // because the key might be nonunique,
                    // we finish porbing untill we get the last hashcell in the linked list.
                    reg_state[j] = state_t::NEXT;
                    break;
                default:
                    break;

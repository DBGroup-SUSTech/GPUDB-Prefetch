...
__launch_bounds__(128, 1)  //
    __global__ void gets_4(int64_t *keys, int n, int64_t *values,
  ...
  for (int k = 0; k < PDIST_4; ++k) fsm[k].state[lid] = state_t::INIT;
...
__launch_bounds__(32, 1)  //
    __global__ void gets_2(int64_t *keys, int n, int64_t *values,
                           const Node *const *root_p) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = blockDim.x * gridDim.x;
  int i = tid;
...
__launch_bounds__(32, 1)  //
    __global__ void gets_3(int64_t *keys, int n, int64_t *values,
                           const Node *const *root_p) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = blockDim.x * gridDim.x;
  int i = tid;

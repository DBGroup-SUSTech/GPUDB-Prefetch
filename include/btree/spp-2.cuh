...
constexpr int LANES_PER_BLOCK = MACRO_BLOCKSIZE / 32 * LANES_PER_WARP; // 32
  ...
  for (int i = 0; ; i ++) {
    ...
    for (int j = i >= G ? G - 1 : i; j >= 0; j--) {
      ...
      if (node->type == Node::Type::INNER) {
        ...
        int pos = inner->lower_bound(key[jmod]);
        ...
        int pos = leaf->lower_bound(key[jmod]);
        if (pos < leaf->n_key && key[jmod] == leaf->keys[pos]) {
